...
const AnimalGroups = () => {
    ...
    const fetchCounts = async () => {
        try {
            const [animalsResponse, bloodSamplesResponse] = await Promise.all([
                axiosInstance.get('/animals/groups'),
                axiosInstance.get('/animals/blood-samples')
            ]);

            const animals = animalsResponse.data.data || [];
            const bloodSamples = bloodSamplesResponse.data.data || [];

            setCounts({
                breeding: animals.filter(a => a.purpose === 'damızlık').length,
                negativeTest: animals.filter(a => a.test_result === 'NEGATIF').length,
                pendingTest: bloodSamples.filter(s => s.status === 'BEKLEMEDE').length
        ...
        try {
            ...
            switch (type) {
                ...
                case 'Test Sonucu Negatif':
                    ...
                    break;
                case 'Test Bekleyenler':
                    response = await axiosInstance.get('/animals/blood-samples?status=BEKLEMEDE');
                    break;
                case 'Kesilenler':
                ...
                <Card>
                    <CardContent sx={{ textAlign: 'center' }}>
                        ...
                        <Button
                            variant="contained"
                            color="warning"
                            onClick={() => handleShowAnimals('Test Bekleyenler')}
                            sx={{ mt: 2 }}
                            fullWidth
                        >
                            Hayvanları Göster
                            ...
                            <tbody>
                                {modalAnimals.map((sample) => (
                                    <tr key={sample.id} style={{ borderBottom: '1px solid #ddd' }}>
                                        <td style={{ padding: '12px', color: 'rgba(0, 0, 0, 0.87)' }}>{sample.animal_id || '-'}</td>
                                        <td style={{ padding: '12px', color: 'rgba(0, 0, 0, 0.87)' }}>{sample.status || '-'}</td>
                                        <td style={{ padding: '12px', color: 'rgba(0, 0, 0, 0.87)' }}>{sample.created_at ? new Date(sample.created_at).toLocaleDateString() : '-'}</td>
                                    </tr>
                                ))}
                            </tbody>
                        </table>
