...
const columns = [
  ...
  { field: 'test_name', headerName: 'Test Adı', width: 200 },
  { field: 'test_date', headerName: 'Test Tarihi', width: 180 },
  { field: 'test_result', headerName: 'Test Sonucu', width: 150 },
  // ... diğer gerekli kolonlar
];

const TestedAnimalsList = () => {
  const [rows, setRows] = useState([]);
  const [loading, setLoading] = useState(false);
  const [exporting, setExporting] = useState(false);

  useEffect(() => {
    const fetchTestedAnimals = async () => {
      setLoading(true);
      try {
        const response = await axios.get('/api/v1/reports/tested-animals');
        const processedData = response.data.map((row, index) => ({
          id: row.id || row.test_id || index,
          ...row,
        }));
        setRows(processedData);
        toast.success('Test yapılan hayvan verileri başarıyla yüklendi.');
      } catch (error) {
        console.error('Test verileri alınırken hata oluştu:', error);
  ...
  const handleExportPDF = () => {
    ...
    try {
      const doc = new jsPDF();
      doc.text('Test Yapılan Hayvanlar Listesi', 14, 16);
      doc.autoTable({
        startY: 20,
      ...
      doc.save('test_yapilan_hayvanlar_listesi.pdf');
  ...
  const handleExportExcel = () => {
    ...
    try {
      ...
      XLSX.utils.book_append_sheet(workbook, worksheet, 'Test Yapılan Hayvanlar');
      const colsWidths = columns.map(col => ({ wch: col.headerName.length + 5 }));
      worksheet['!cols'] = colsWidths;
      XLSX.writeFile(workbook, 'test_yapilan_hayvanlar_listesi.xlsx');
...
export default TestedAnimalsList;
