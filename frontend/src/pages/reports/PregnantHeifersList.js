...
const columns = [
  ...
  { field: 'insemination_date', headerName: 'Tohumlama Tarihi', width: 180 },
  { field: 'expected_birth_date', headerName: 'Tahmini Doğum Tarihi', width: 200 },
...
const PregnantHeifersList = () => {
  ...
  useEffect(() => {
    const fetchPregnantHeifers = async () => {
      setLoading(true);
      try {
        const response = await axios.get('/api/v1/reports/pregnant-heifers');
        const processedData = response.data.map((row, index) => ({
          id: row.id || index,
          ...row,
        }));
        setRows(processedData);
        toast.success('Gebe düve verileri başarıyla yüklendi.');
      } catch (error) {
  ...
  const handleExportPDF = () => {
    ...
    try {
      const doc = new jsPDF();
      doc.text('Gebe Düveler Listesi', 14, 16);
      doc.autoTable({
        startY: 20,
  ...
  const handleExportExcel = () => {
    ...
    try {
      ...
      XLSX.writeFile(workbook, 'gebe_duveler_listesi.xlsx');
