...
const columns = [
  ...
  { field: 'last_birth_date', headerName: 'Son Doğum Tarihi', width: 180 },
  { field: 'dry_off_date', headerName: 'Kuruya Ayrılma Tarihi', width: 200 },
  { field: 'expected_calving_date', headerName: 'Beklenen Buzağılama Tarihi', width: 220 },
...
const DryCowsList = () => {
  ...
  useEffect(() => {
    const fetchDryCows = async () => {
      setLoading(true);
      try {
        const response = await axios.get('/api/v1/reports/dry-cows'); // Gerçek endpoint ile değiştirin
        const processedData = response.data.map((row, index) => ({
          id: row.id || index,
          ...row,
        }));
        setRows(processedData);
        toast.success('Kuru inek verileri başarıyla yüklendi.');
      } catch (error) {
  ...
  const handleExportPDF = () => {
    ...
    try {
      const doc = new jsPDF();
      doc.text('Kuru İnekler Listesi', 14, 16);
      doc.autoTable({
        startY: 20,
        head: [columns.map(col => col.headerName)],
        body: rows.map(row => columns.map(col => row[col.field] ?? '-')),
        theme: 'grid',
        headStyles: { fillColor: [231, 76, 60] }, // Farklı bir başlık rengi
      });
