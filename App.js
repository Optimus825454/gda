...
import { Routes, Route, useNavigate } from 'react-router-dom'; // useNavigate eklendi
...
import { LoadingProvider, useLoading } from './contexts/LoadingContext'; // useLoading da import edildi
import GlobalLoader from './components/common/GlobalLoader'; // GlobalLoader import edildi
import UserManagement from './pages/user/UserManagement';
import { setLoadingHooks } from './utils/axiosConfig'; // setLoadingHooks import edildi
...
import TestList from './pages/health/TestList';
...
import DeathEntry from './pages/animal/DeathEntry'; // Yeni eklenen Ölüm Girişi sayfası
import DeadAnimalList from './pages/animal/DeadAnimalList'; // Yeni eklenen Ölen Hayvanlar Listesi sayfası
...
function AppContent() {
    ...
    const { showLoading, hideLoading } = useLoading(); // Loading hookları alındı

    // Axios interceptorları için loading hooklarını ayarla
    useEffect(() => {
        setLoadingHooks(showLoading, hideLoading);
    }, [showLoading, hideLoading]);

    if ( authLoading ) {
        // Auth yüklenirken GlobalLoader'ı kullanabiliriz veya eski spinner kalabilir.
        // showLoading('Oturum doğrulanıyor...'); // Örnek kullanım
        return (
        ...
        // Eğer auth bitti ama hala global bir yükleme varsa (API isteği gibi)
        // GlobalLoader zaten görünecek.
    }*/
            ...
            <Routes>
                ...
                {user ? (
                    <Route path="/" element={<MainLayout />}>
                        ...
                        {/* Hayvan Yönetimi */}
                        <Route path="animals" element={<PrivateRoute element={<AnimalList />} permissionCode="view_animals" />} />
                        ...
                        <Route path="animals/death" element={<PrivateRoute element={<DeathEntry />} permissionCode="MANAGE_ANIMAL" />} /> {/* Ölüm Girişi rotası */}
                        <Route path="animals/dead" element={<PrivateRoute element={<DeadAnimalList />} permissionCode="READ_ANIMAL" />} /> {/* Ölen Hayvanlar Listesi rotası */}
