...
const AllCowsList = () => {
  ...
  const columns = [
    ...
    { id: 'breed', label: 'Irkı', minWidth: 150 },
    ...
    { id: 'lactation_number', label: 'Laktasyon No', minWidth: 130, type: 'number' },
    { id: 'tohumlama_tarihi', label: 'Son Tohumlama', minWidth: 150, type: 'date', format: formatDate },
    { id: 'last_birth_date', label: 'Son Doğum', minWidth: 150, type: 'date', format: formatDate }
  ];
  ...
  useEffect( () => {
    const fetchAllCows = async () => {
      ...
      setError( null );
      try {
        const response = await axiosInstance.get( '/animals/reports/all-cows' );
        console.log( 'Tüm inekler:', response.data );
    ...
    try {
      ...
      doc.autoTable( {
        ...
        didDrawPage: ( data ) => {
          doc.setFontSize( 18 );
          doc.setTextColor( 40 );
          doc.text( 'Tüm Hayvanlar Listesi Raporu', data.settings.margin.left, 15 ); // PDF Başlığı (İhtiyaç duyulursa değiştirin)
        },
      } );

      doc.save( 'tum_hayvanlar_listesi.pdf' );
  ...
  return (
    <Paper sx={{ p: 3, width: '100%' }}>
      ...
      <CustomTable
        ...
        defaultPageSize={50}
        emptyMessage="Hiç inek kaydı bulunamadı"
      />
    </Paper>
  );
};
