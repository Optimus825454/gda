...
// Tahmini doğum tarihi hesaplama fonksiyonu (inek için ~280 gün)
const calculateExpectedBirthDate = ( inseminationDate ) => {
  ...
  try {
    ...
    // Tohumlama tarihine 280 gün ekle (yaklaşık 9 ay, inek gebelik süresi)
...
const PregnantCowsList = () => {
  ...
  useEffect( () => {
    const fetchPregnantCows = async () => {
      ...
      setError( null );
      try {
        const response = await axiosInstance.get( '/animals/reports/pregnant-cows' );
        ...
        if ( response.data && response.data.data ) {
          processedData = response.data.data.map( ( animal, index ) => {
            if ( animal.id === null || animal.id === undefined ) {
              console.warn( `Hayvan verisinde ID eksik veya geçersiz. Index: ${index}`, animal );
            }
            return {
              id: animal.id, // Sadece backend'den gelen ID'yi kullan
              ...animal
            };
          } );
  ...
  const handleExportPDF = () => {
    ...
    try {
      ...
      // Türkçe karakterler için font ekleme denemesi
      // jsPDF'in standart fontları genellikle bu tür sorunları çözer, 
      // ancak karmaşık Türkçe karakterler için özel bir font dosyası (örn. .ttf) gerekebilir.
      // Şimdilik 'Helvetica' veya 'Arial' gibi yaygın bir fontu deneyebiliriz.
      // Eğer bu işe yaramazsa, özel font yükleme yöntemine geçilir.
      ...
      doc.autoTable( {
        ...
        body: tableRows,
        startY: 20,
        styles: { font: 'Helvetica', fontStyle: 'normal' }, // Fontu burada da belirtmek önemli
        headStyles: { fillColor: [22, 160, 133] },
        didDrawPage: ( data ) => {
          // Başlık
          doc.setFontSize( 18 );
          doc.setTextColor( 40 );
          doc.text( 'Gebe İnekler Listesi Raporu', data.settings.margin.left, 15 );
        },
      } );
  ...
  return (
    <Paper sx={{ p: 3, width: '100%' }}>
      ...
      <CustomTable
        ...
        defaultSortBy="tohumlama_tarihi"
        defaultSortOrder="desc"
        defaultPageSize={50}
        emptyMessage="Hiç gebe inek kaydı bulunamadı"
      />
    </Paper>
  );
};
