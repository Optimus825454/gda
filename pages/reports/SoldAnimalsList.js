...
const SoldAnimalsList = () => {
  ...
  const columns = [
    ...
    { id: 'animal_id', label: 'Hayvan ID', minWidth: 150 },
    { id: 'ear_tag', label: 'Küpe No', minWidth: 150 },
    { id: 'name', label: 'Adı', minWidth: 150 },
    { id: 'sale_date', label: 'Satış Tarihi', minWidth: 180, type: 'date', format: formatDate },
    { id: 'sale_price', label: 'Satış Fiyatı', minWidth: 150, type: 'number', format: formatPrice },
    { id: 'buyer', label: 'Alıcı', minWidth: 200 },
    { id: 'notes', label: 'Notlar', minWidth: 250 }
  ];
  ...
  useEffect(() => {
    const fetchSoldAnimals = async () => {
      ...
      setError(null);
      try {
        const response = await axiosInstance.get('/animals/reports/sold-animals');
    ...
    try {
      ...
      doc.autoTable({
        ...
        didDrawPage: (data) => {
          doc.setFontSize(18);
          doc.setTextColor(40);
          doc.text('Satılan Hayvanlar Listesi Raporu', data.settings.margin.left, 15); // PDF Başlığı
        },
      });
  ...
  return (
    <Paper sx={{ p: 3, width: '100%' }}>
      ...
      <CustomTable 
        ...
        defaultPageSize={50}
        emptyMessage="Satılan hayvan kaydı bulunamadı"
      />
    </Paper>
  );
};
